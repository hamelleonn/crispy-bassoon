...
export const ProductAbout: React.FC<Props> = ({ productDescr }) => {
  return (
    <div className="product__about" data-cy="productDescription">
      ...
      {productDescr.map(({ title, text }: ProductDescrType) => (
        <div key={getId()} className="product__about-descr-wrapper">
          <h4 className="product__about-descr-title">{title}</h4>

          {text.map(textEl => (
            <p className="product__about-descr" key={getId()}>
              {textEl}
            </p>
          ))}
        </div>
