...
export const PhonesProvider: React.FC<Props> = ({ children }) => {
  ...
  const preparedHotPriceProducts =
    products?.filter(
      (product: Product) => product.fullPrice - product.price >= 90,
    ) || [];

  const preparedBrandNewProducts =
    products?.filter((product: Product) => product.year >= 2019) || [];
